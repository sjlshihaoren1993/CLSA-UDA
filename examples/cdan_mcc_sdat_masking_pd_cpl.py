...
def main(args: argparse.Namespace):
    ...
    if args.phase == 'test':
        acc1 = utils.test(test_loader, classifier, args, device)
        print(acc1)
        return

    # thresholding module with convex mapping function x / (2 - x)
    thresholding_module = DynamicThresholdingModule(0.95, False, lambda x: x / (2 - x), num_classes,
                                                    len(train_target_dataset), device=device)
    ...
    for epoch in range(args.epochs):
        ...
        train(train_source_iter, train_target_iter, classifier, teacher,
              ...
              lr_scheduler, lr_scheduler_ad, epoch, thresholding_module, args)
        # evaluate on validation set
        acc1, samples_indices, pseudolb = utils.validate(val_loader, classifier, args, device)
        if args.log_results:
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          ...
          lr_scheduler: LambdaLR, lr_scheduler_ad, epoch: int, thresholding_module, args: argparse.Namespace):
    ...
    for i in range(args.iters_per_epoch):
        ...
        x_t, _, x_t_filename, idx_t = next(train_target_iter)
        ...
        pseudo_label_t, pseudo_prob_t, _ = teacher(x_t)
        ...
        dynamic_threshold = thresholding_module.get_threshold(pseudo_label_t)
        mask = (pseudo_prob_t > dynamic_threshold).float()
        # mask used for updating learning status
        selected_mask = (pseudo_prob_t > 0.95).long()
        thresholding_module.update(idx_t, selected_mask, pseudo_label_t)

        masking_loss_value = args.trade_off * (
                F.cross_entropy(y_t_masked, pseudo_label_t, reduction='none') * mask).mean()
